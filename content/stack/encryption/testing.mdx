...
1. A PostgreSQL database with [EQL](/stack/platform/eql) installed (see [CipherStash Forge](/stack/encryption/forge))
...
beforeAll(async () => {
  ...
  // Run `npx @cipherstash/cli db setup` (or `npx @cipherstash/cli db install`) against your test database first.
