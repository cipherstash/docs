...
<Callout type="info">
  Install [EQL](/stack/platform/eql) in your Supabase database using [CipherStash Forge](/stack/encryption/forge) so encrypted columns can use the `eql_v2_encrypted` type.
</Callout>
...
<Callout type="info">
  If you installed EQL manually (without `--supabase`), you also need to grant permissions. See [CipherStash Forge — Supabase install](/stack/encryption/forge/install#supabase-install) for the required grants.
</Callout>
