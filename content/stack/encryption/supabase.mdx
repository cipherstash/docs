...
<Callout type="info">
  Install the EQL schema in your Supabase (PostgreSQL) database so encrypted columns can use the `eql_v2_encrypted` type. Apply with **psql** using your database URL, or **download** the SQL file and run it in the Supabase SQL editor.
</Callout>

**Apply with psql:**

```bash
curl -sL https://github.com/cipherstash/encrypt-query-language/releases/download/eql-2.2.1/cipherstash-encrypt-supabase.sql | psql $DATABASE_URL
```

**Download:** [cipherstash-encrypt-supabase.sql](https://github.com/cipherstash/encrypt-query-language/releases/download/eql-2.2.1/cipherstash-encrypt-supabase.sql) — then open the Supabase SQL editor and run the file contents.
...
```sql filename="supabase-grants.sql"
...
Under the hood, `encryptedSupabase` uses a deferred query builder pattern. All chained operations (`.select()`, `.eq()`, `.insert()`, etc.) are recorded synchronously. When the query is `await`ed, the builder:
