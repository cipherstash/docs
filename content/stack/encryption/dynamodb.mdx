---
title: DynamoDB
description: Encrypt and decrypt DynamoDB items with the encryptedDynamoDB helper
---
...
<Callout type="info">
  `encryptedColumn` also supports `.orderAndRange()`, `.freeTextSearch()`, and `.searchableJson()` index methods, but only `.equality()` produces HMAC values usable for DynamoDB key condition queries.
</Callout>

See [Schema definition](/stack/encryption/schema) for full details on index types, data types, and nested objects.
...
See [Error handling](/stack/encryption/error-handling) for details on error codes and handling patterns.
