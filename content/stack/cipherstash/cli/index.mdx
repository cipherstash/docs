...
<Steps>
  ...
  <Step>
    ...
    After install, the CLI prints a next-steps block pointing you to the wizard, direct SDK usage, and docs.
  </Step>
...
The `wizard` command uses AI to generate an encryption schema from your existing database schema.
...
1. Reads your existing database schema and application code.
2. Proposes which columns to encrypt and which search indexes to add.
3. Generates or updates your encryption client file.
4. Scans your source code for Drizzle/Supabase insert, update, and select patterns on encrypted tables. It prints a summary of call sites that need `encryptModel`/`decryptModel` wrappers. The wizard reports these locations but does not edit your application code.
5. Prompts you to install integration-appropriate [agent skills](/stack/reference/agent-skills) into `./.claude/skills/` for your AI coding assistant.

### Wizard log

Each run appends a timestamped log to `.cipherstash/wizard-log.md` in your project root. The log records phases, decisions, and touched files. Review it to understand what the wizard changed and why.
...
<Cards>
  <Card title="Init wizard" href="/stack/cipherstash/cli/init" description="Use the interactive wizard to set up the CLI in your project." />
