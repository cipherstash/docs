...
description: Query encrypted data without decryption. Equality, free-text search, range, ordering, and JSON queries over ciphertext in PostgreSQL.
---
...
<Callout type="info">
  Order by the `eql_v2.ore_block_u64_8_256()` extraction function, not the bare column — a bare `ORDER BY` does not match the functional range index. Range indexes are self-hosted only; `ORDER BY` on an encrypted column is not supported on Supabase.
</Callout>
