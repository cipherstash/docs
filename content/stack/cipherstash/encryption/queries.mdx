...
`SELECT DISTINCT` on an encrypted column deduplicates by plaintext. The stored ciphertext is randomized per row, so identical plaintexts would not otherwise collapse — CipherStash Proxy rewrites the query so they do. No query changes are needed.
...
`SELECT DISTINCT ... ORDER BY <encrypted column>` is also supported, with no query changes needed.
...
<Callout type="warn">
  Deduplication is equality-based. A column with no equality index — most commonly a storage-only column with no `.equality()`, `.freeTextSearch()`, or `.orderAndRange()` configured — cannot be deduplicated. `SELECT DISTINCT` on that column raises a capability error.
</Callout>

### Unsupported shapes

Two combinations of `DISTINCT` and `ORDER BY` on an encrypted column are not supported and raise an error:

**`DISTINCT ON (...)` combined with `ORDER BY`:**

```sql
-- Not supported
SELECT DISTINCT ON (email) * FROM users ORDER BY email;
```

**`DISTINCT *` combined with `ORDER BY`, or with an encrypted column anywhere in the projection:**

```sql
-- Not supported: DISTINCT * with ORDER BY on an encrypted column
SELECT DISTINCT * FROM users ORDER BY email;

-- Not supported: an encrypted column reached via `*` in the projection
...
```sql
SELECT DISTINCT id, email FROM users ORDER BY email;
```
