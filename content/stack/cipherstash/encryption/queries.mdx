...
// the functional range index expression. Requires a range index, so this
// is self-hosted only.
const result = await pgClient.query(
...
// Sort (requires a range index; self-hosted only)
const results = await db
  .select()
  .from(usersTable)
...
<Callout type="warn">
  `ORDER BY` on encrypted columns needs the range (Block-ORE) index, which is self-hosted only — its operator class requires superuser to install, and Supabase does not grant superuser. Range _filters_ (`>`, `>=`, `<`, `<=`) return correct results on both self-hosted and Supabase. On Supabase, `ORDER BY` on an encrypted column is not supported — sort application-side after decrypting.
</Callout>
