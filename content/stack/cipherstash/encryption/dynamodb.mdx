...
<Faq
  items={[
    faqPrivacy,
    {
      title: "Can I query encrypted DynamoDB attributes?",
      answer: (
        <>
          Yes, for exact equality. Each encrypted attribute stores an{" "}
          <code>__hmac</code> term you match against, so you can look up by an
    ...
    },
    {
      ...
      answer: (
        <>
          No. <code>encryptedDynamoDB</code> encrypts items before writing and
    ...
    },
    {
      ...
      answer: (
        <>
          Install <code>@cipherstash/stack</code>, define an encrypted schema for
