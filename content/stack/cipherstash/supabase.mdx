...
| `@cipherstash/cli` | Setup and schema management. Installs EQL into your database, scaffolds your encryption client, and generates migrations when your encryption schema changes. Comparable to Prisma CLI or Drizzle Kit. | devDependency |
...
<Steps>
  ...
  <Step>
    ...
    - Authenticates you with CipherStash via your browser.
    - Asks how your application connects to Supabase (JS SDK, Drizzle, Prisma, or raw SQL).
    - Generates an encryption client at `./src/encryption/index.ts` that your code imports.
    - Installs `@cipherstash/forge` and `@cipherstash/stack` if they aren't already present.

    The `--supabase` flag selects the Supabase JS SDK setup flow.
  ...
  <Step>
    ...
    <Tabs items={["As a migration (recommended)", "Direct push"]}>
      <Tab value="As a migration (recommended)">
