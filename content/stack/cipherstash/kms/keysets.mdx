...
Key Sets are the foundational primitive that powers [multi-tenant encryption](/stack/cipherstash/encryption/configuration#keysets) in the Encryption SDK and [environment isolation](/stack/cipherstash/secrets/concepts#environments) in Secrets.
...
See [Secrets concepts](/stack/cipherstash/secrets/concepts#environments) for more detail.
...
- **Environment isolation:** Use different keysets for production, staging, and development. In the Dashboard, manage these under **Vault > Environments**. See [Secrets concepts](/stack/cipherstash/secrets/concepts#environments).
