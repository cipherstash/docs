...
Navigate to **Clients** in the Dashboard to see all active clients, including device-backed clients created by `npx stash init`.
...
If a developer works on more than one machine (e.g., a laptop and a desktop), they run `npx stash init` on each machine.
