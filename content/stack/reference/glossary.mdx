...
- **Device-backed clients** are created automatically by `npx stash init` and tied to a developer's [device](#device). Used for local development.
...
A unique identity tied to a developer's machine and user account, created by `npx stash init`.
