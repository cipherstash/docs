...
<Callout type="info">
Prometheus metrics are exposed on port 9930. Read more about them in the [reference documentation](/stack/proxy/reference#prometheus-metrics).
</Callout>
...
For local development, create a client and access key in the [Dashboard](https://dashboard.cipherstash.com) and set them in your `.env.proxy.docker` file.
For production, see [Going to production](/stack/guides/going-to-production) to set up machine client credentials.
</Callout>
...
See the [Proxy config reference](/stack/proxy/reference) for all available options.

## Setting up the database schema

Proxy uses [EQL](/stack/platform/eql) to index and search encrypted data.
...
See the [CLI reference](/stack/proxy/reference#cli-reference) for usage details.

## Reference

See the [CipherStash Proxy reference](/stack/proxy/reference) for all available options.
