...
CipherStash uses [keysets](/stack/kms/keysets) to scope encryption keys:
...
7. SDK packages ciphertext + index tokens into a [CipherCell](/stack/platform/cipher-cell) (JSON payload).
...
4. PostgreSQL's [EQL extension](/stack/platform/eql) compares encrypted tokens using the appropriate index — the database never sees plaintext.
...
For a deeper analysis of each index type, see [Searchable encryption](/stack/platform/searchable-encryption) and [Supported queries](/stack/platform/supported-queries).
...
CipherStash services are deployed across [multiple AWS regions](/stack/kms/regions). Key material does not leave the region your workspace is configured in.
...
- [What is CipherStash?](/stack/platform/what-is-cipherstash) — platform overview and threat model
- [ZeroKMS](/stack/kms) — key management and keyset isolation
- [CipherCell format](/stack/platform/cipher-cell) — encrypted payload structure
- [Disaster recovery](/stack/kms/disaster-recovery) — key recovery and outage behavior
