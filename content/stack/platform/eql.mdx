...
**Encrypt Query Language (EQL)** is a set of PostgreSQL types, operators, and functions that enable queries on encrypted data without decryption. EQL works seamlessly with the [CipherCell](/stack/platform/cipher-cell) format to provide searchable encryption capabilities directly in PostgreSQL.
...
When combined with the [Encryption SDK](/stack/encryption) or [CipherStash Proxy](/stack/proxy), EQL enables:
...
The foundation of EQL is the `eql_v2_encrypted` data type, which stores [CipherCells](/stack/platform/cipher-cell) containing encrypted data and searchable encrypted metadata.
...
[Learn more about exact indexes](/stack/platform/supported-queries#exact-match)

### `ore` — Range queries

Enables range comparisons (`<`, `>`, `BETWEEN`) and ordering (`ORDER BY`) using Order Revealing Encryption.

[Learn more about range indexes](/stack/platform/supported-queries#range--order)

### `match` — Pattern matching

Enables substring and full-text search (`LIKE`, `ILIKE`) using encrypted Bloom filters with trigrams.

[Learn more about match indexes](/stack/platform/supported-queries#match-pattern)
...
EQL leverages PostgreSQL's native indexing capabilities to enable efficient queries on encrypted data. The searchable encrypted metadata within [CipherCells](/stack/platform/cipher-cell) is indexed using standard PostgreSQL index types (B-tree for exact/range, GIN for pattern matching).
...
- [CipherCell format](/stack/platform/cipher-cell) — The data structure used by EQL
- [Supported queries](/stack/platform/supported-queries) — Available searchable encryption schemes
