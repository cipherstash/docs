...
CipherStash uses a split-key architecture. [ZeroKMS](/stack/kms) manages an authority key that is necessary but not sufficient to derive data keys. Your application holds an independent client key. Data keys are derived locally and never leave your infrastructure.
...
- **[EQL](/stack/platform/eql)** provides native PostgreSQL types for encrypted data
...
|---|---|---|
| [Encryption SDK](/stack/encryption) | Application-layer encryption with fine-grained control | Moderate (encrypt/decrypt calls in your data layer) |
| [CipherStash Proxy](/stack/proxy) | Existing PostgreSQL applications with minimal code changes | Minimal (point your connection at the proxy) |

### ORM support

The Encryption SDK integrates with popular ORMs:

- **[Drizzle ORM](/stack/encryption/drizzle)**: Custom encrypted column types with query operators
- **[Supabase](/stack/encryption/supabase)**: Transparent encryption wrapper around the Supabase client
- **[DynamoDB](/stack/encryption/dynamodb)**: Encrypted attributes with searchable capabilities

## ZeroKMS overview

[ZeroKMS](/stack/kms) is CipherStash's zero-trust key management service:

- **Key hierarchy**: Authority keys, client keys, and derived data keys provide defense in depth
- **Regional deployment**: Available in [multiple regions](/stack/kms/regions) for data sovereignty
- **Audit logging**: Complete visibility into key usage and access patterns
- **Disaster recovery**: Multi-region key replication with [automated failover](/stack/kms/disaster-recovery)

## Development to production journey

| Stage | Auth model | Setup |
|---|---|---|
| Local development | Device-based | `npx @cipherstash/stack init` per developer — no env vars needed |
...
See [Going to production](/stack/guides/going-to-production) for the full transition guide and [Team onboarding](/stack/guides/team-onboarding) for setting up your development team.

## Enterprise features

### Lock contexts

[Lock contexts](/stack/encryption/identity) provide identity-aware encryption. Bind encryption operations to authenticated users using JWTs for:
...
1. **Get started**: Run `npx @cipherstash/stack init` to set up [device-based auth](/stack/guides/getting-started)
2. **Choose your integration path**: [Encryption SDK](/stack/encryption/getting-started) or [CipherStash Proxy](/stack/proxy/getting-started)
3. **Build locally**: Device auth handles credentials automatically — focus on your integration
4. **Go to production**: Set up [machine clients and env vars](/stack/guides/going-to-production) for deployment
