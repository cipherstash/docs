...
Local development uses device-based authentication — each developer is uniquely identified via `npx @cipherstash/cli init`.
...
|---|---|---|
| Local development | Device-based | `npx @cipherstash/cli init` creates a device and client. No env vars needed. |
...
If you have `CS_*` variables in a local `.env` file, remove them and run `npx @cipherstash/cli init` instead.
</Callout>
