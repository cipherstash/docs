...
Local development uses device-based authentication — each developer is uniquely identified via `npx @cipherstash/stack init`.
...
|---|---|---|
| Local development | Device-based | `npx @cipherstash/stack init` creates a device and client. No env vars needed. |
...
<Steps>
  ...
  <Step>
    ...
    See [Access keys](/stack/kms/access-keys) for details on roles and scopes.
  </Step>
  ...
  <Step>
    ...
    See [Keysets](/stack/kms/keysets) for details on managing keyset access.
  </Step>
...
If you have `CS_*` variables in a local `.env` file, remove them and run `npx @cipherstash/stack init` instead.
</Callout>
...
See [Bundling](/stack/encryption/bundling) for setup instructions for Next.js, webpack, esbuild, and SST.

## Next steps

- [Bundling for production](/stack/encryption/bundling) — Configure your bundler for deployment
- [Deploying Proxy to AWS ECS](/stack/proxy/aws-ecs) — Run CipherStash Proxy in production
- [Access keys](/stack/kms/access-keys) — Understand access key roles and scopes
- [Team onboarding](/stack/guides/team-onboarding) — Set up per-developer access for your team
