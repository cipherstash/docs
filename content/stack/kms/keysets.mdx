---
title: Keysets
description: Cryptographic isolation for tenants, environments, and custom boundaries
---
...
When a developer runs `npx @cipherstash/stack init`, their device-backed client is automatically granted access to the default keyset. This means no additional configuration is needed for local development.
...
In the CipherStash Dashboard, the **Vault > Environments** section manages these mappings. Creating an environment either creates a new keyset automatically or lets you select an existing one. Each environment row shows its associated keyset ID.
...
- **Environment isolation:** Use different keysets for production, staging, and development. In the Dashboard, manage these under **Vault > Environments**. See [Secrets concepts](/stack/secrets/concepts#environments).
