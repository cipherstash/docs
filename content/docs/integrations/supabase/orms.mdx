---
title: Data layer
description: "Two ways to reach your Supabase data with CipherStash: the encryptedSupabase wrapper over PostgREST, or a direct Postgres connection through an ORM like Drizzle or Prisma."
type: guide
...
| --- | --- | --- |
| **Client** | `supabase-js` (+ `encryptedSupabase`) | Drizzle, Prisma, or raw `pg` |
| **Transport** | Supabase's REST API (PostgREST) | A Postgres connection (pooler or direct) |
| **How encryption plugs in** | The [`encryptedSupabase`](/reference/stack/supabase) wrapper encrypts/decrypts around each request | The [Stack SDK](/reference/stack) encrypts values in your app; EQL-typed columns store and search them |
...
- The **`encryptedSupabase` / PostgREST path** carries the end user's JWT, so `auth.uid()` and your policies apply automatically.
...
<Cards>
  <Card title="Quickstart" href="/integrations/supabase/quickstart">
    The PostgREST path end-to-end with encryptedSupabase.
  </Card>
