...
| `.contains()` | Plaintext JSON or array only on EQL 3.0.2 | Uses native PostgREST containment |
...
<Callout type="warn">
**EQL 3.0.2 cannot run encrypted free-text or encrypted JSON queries through supabase-js.** `.matches()`, encrypted `.contains()`, `.selectorEq()`, and `.selectorNe()` fail before a request is sent. Their SQL operators require an `eql_v3.query_*` cast that PostgREST cannot express.
