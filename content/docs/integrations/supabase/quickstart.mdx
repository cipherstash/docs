...
<Callout type="warn">
`.like()` and `.ilike()` throw on encrypted columns. Free-text matching is `.contains()`, which compares encrypted token sets rather than SQL wildcards. **Substring matching is currently broken**: today `.contains()` matches only the full stored value, or a three-character search string. See the [wrapper reference](/reference/stack/supabase#there-is-no-like).
</Callout>
