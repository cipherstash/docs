...
The install targets EQL v3 by default. [Fundamentals](/integrations/supabase/fundamentals#installing-eql) covers the rest, including why a v3 install does not survive `supabase db reset`.
...
// Free-text match: encrypted token matching, not SQL LIKE
await db.from("patients").select("id, name").matches("name", "Alice Chen")
...
<Callout type="warn">
Free-text matching is `.matches()`, which compares encrypted token sets rather than SQL wildcards. `.contains()` raises on an encrypted text column — it promises exactness that bloom matching cannot deliver. `.like()` and `.ilike()` still run, but only as an approximate shim over `.matches()`. See the [wrapper reference](/reference/stack/supabase#free-text-search-matches).
</Callout>
