...
The install targets **EQL v3 by default**; `--eql-version 2` opts back to v2. The full flag set is in the [CLI reference](/reference/cli/eql).

<Callout type="warn">
EQL v3 installs **directly against the database only**. `--migration`, `--drizzle`, `--latest`, and `--migrations-dir` are rejected for a v3 install; they exist for the v2 install paths.
