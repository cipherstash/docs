...
flowchart TD
    ...
    client -- derivation input --> seed
    seed -- "layer 2:<br/>HMAC-based KDF" --> datakey
    datakey -- AES-256-GCM-SIV --> ciphertext
```

Re-encryption describes how the seed is produced. HMAC key derivation describes how the seed becomes per-value keys. Both are true, at different layers.

Note where the boundary falls: the key seed crosses it, and the data key never does.
