...
That leakage model is documented once, in [Searchable encryption](/how-it-works/searchable-encryption), with per-term detail and guidance on when the trade is not worth making. Assess it as part of your threat model. If the ordering or frequency of a column's values is itself sensitive, encrypt that column without a searchable index and filter after decryption.
...
- [Searchable encryption](/how-it-works/searchable-encryption) for the canonical index-term leakage model.
