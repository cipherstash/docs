...
<Callout type="warn">
`new LockContext()` and `lockContext.identify(jwt)` are **deprecated**. Per-operation CTS tokens were removed in `protect-ffi` 0.25, and `identify()` no longer affects encryption. Authenticate the client with `OidcFederationStrategy` and pass the claim to `.withLockContext()` instead.
</Callout>
