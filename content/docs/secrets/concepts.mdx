...
**Environments** provide cryptographic isolation within a workspace. Each environment maps to its own [Key Set](/docs/kms/keysets) in ZeroKMS, so secrets in one environment cannot be decrypted with keys from another. This is the same Key Set primitive used for [multi-tenant encryption](/docs/encryption/configuration#keysets) in the Encryption SDK.
...
- **Environment isolation** — Each environment uses a separate [Key Set](/docs/kms/keysets). A secret encrypted in staging cannot be decrypted with production keys. This is the same primitive used for [multi-tenant encryption](/docs/encryption/configuration#keysets).
