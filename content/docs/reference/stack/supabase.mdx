...
<Callout type="warn">
**Substring matching is currently broken.** The search string is tokenized as though it were a stored value, so its bloom filter carries the whole search string as an extra token. The stored value's bloom has no such token, so containment fails: `contains("email", "alice")` finds nothing on a stored `alice@example.com`.

In practice `.contains()` matches only when the search string equals the stored value, or is exactly three characters, the trigram length. This is a defect rather than intended behaviour, and a fix is in progress.
</Callout>
