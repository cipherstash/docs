...
`encryptedSupabase` is the EQL v2 dialect. It ships from the same `@cipherstash/stack-supabase` package and is unchanged from before the 1.0 split. It is synchronous, takes `{ encryptionClient, supabaseClient }`, requires `.from(tableName, schema)` with the schema as a second argument, rejects `select('*')`, and uses `.like()` / `.ilike()` for free-text search. Pick the dialect by function: v2 targets the EQL v2 composite type, v3 targets the `public.eql_v3_*` domains. New work should use `encryptedSupabaseV3`.
