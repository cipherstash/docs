...
<Callout type="warn">
`.contains()` on a `text_search` or `text_match` column tests whether the stored value's encrypted token set contains the query's encrypted token set. With the default match options, the query term includes the whole query string as one of its tokens, so a **substring does not match**: `contains("email", "alice")` finds nothing on a stored `alice@example.com`. Match against the full value. Matching is case-insensitive, and false positives are possible while false negatives are not.
</Callout>
