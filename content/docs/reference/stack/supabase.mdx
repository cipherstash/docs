...
In practice `.contains()` matches only when the search string equals the stored value, or is exactly three characters (the trigram length), where its `include_original` token coincides with a trigram the stored value already has. The v3 Drizzle `contains` has the same defect, for the same reason. A fix belongs in EQL and protect-ffi, not in the wrapper.
</Callout>
