...
The cause is that `text_search` columns always build their match index with `include_original` on, and expose no way to turn it off. The needle is tokenized as if it were a stored value, so its bloom carries the whole needle as an extra token, and the stored value's bloom has no such token. Tracked in [cipherstash/stack#610](https://github.com/cipherstash/stack/issues/610).
</Callout>
