...
- `public.<T>_ord` declares *orderable* and leaves the mechanism to EQL's default.
- `public.<T>_ord_ore` declares *orderable via ORE* (order-revealing encryption), explicitly. Its term is `ob`, extracted with `eql_v3.ord_term`.
- `public.<T>_ord_ope` declares *orderable via OPE* (order-preserving encryption), explicitly. Its term is `op`, extracted with `eql_v3.ord_ope_term`.
