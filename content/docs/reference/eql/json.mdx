...
description: "Store and query encrypted JSON documents with eql_v3.json — containment, field access, and path queries over ciphertext, with the native jsonb operators that don't apply blocked outright."
type: reference
components: [eql]
verifiedAgainst:
  eql: "3.0.0"
...
The full wire shape of each is documented in [Payload format](/reference/eql/payload-format).
...
<Callout type="warn">
**Type your operands.** `eql_v3.json` is a domain over `jsonb`, and PostgreSQL resolves `domain OP untyped_literal` to the **native** `jsonb` operator — bypassing both the encrypted operator and the blockers. `WHERE doc -> 'email'` silently runs native `jsonb ->` and returns `NULL`; `WHERE doc -> 'email'::text` resolves the encrypted operator. This is the same rule as the [scalar operators](/reference/eql/operators). Queries through CipherStash Proxy always bind typed parameters, so this only bites hand-written ad-hoc SQL.
</Callout>
...
<Cards>
  <Card title="Payload format" href="/reference/eql/payload-format">
    The wire shape of the ste_vec envelope and its entries.
  </Card>
  ...
  <Card title="Operators" href="/reference/eql/operators">
    The full operator surface, including the typed-operand rule.
  </Card>
</Cards>
