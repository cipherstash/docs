...
| `public.jsonb_query` | A containment needle: entries with selectors and index terms but **no ciphertext**. This is what you cast a `@>` operand to. |
...
A document payload for an `public.json` column:
...
A containment **query** payload (`public.jsonb_query`) has the same `sv` shape but its entries carry no `c` — containment matches selectors and index terms, never ciphertexts. This is the needle the client builds for a `@>` query:
...
`@>` tests whether the encrypted document contains a structure; `<@` is the reverse. Build the needle with the client and cast it to `public.jsonb_query` (a typed `public.json` or `public.jsonb_entry` operand also works):

```sql
SELECT * FROM orders
WHERE metadata @> $1::public.jsonb_query;
```
