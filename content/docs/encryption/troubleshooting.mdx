...
**Fix**: Ensure the client is configured with the same keyset that was used during encryption. Check the `keyset` option in your config or verify which keyset your client is associated with in the [dashboard](/docs/platform/clients).

### Lock context mismatch

If data was encrypted with [identity-aware encryption](/docs/encryption/identity), it can only be decrypted with the same user's identity.
...
Searchable encryption requires the [EQL PostgreSQL extension](/docs/concepts/eql). Without it, queries against encrypted columns won't work.
...
**Fix**: See [Bundling](/docs/encryption/bundling) for configuration for Next.js, webpack, esbuild, and SST.
