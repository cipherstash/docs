...
```typescript title="__mocks__/encryption.ts"
...
beforeAll(async () => {
  ...
  if (client.failure) {
    throw new Error(`Test setup failed: ${client.failure.message}`)
  }
})
...
```typescript title="test/encryption.test.ts"
test("encrypt and decrypt returns original value", async () => {
  const original = "alice@example.com"

  const encrypted = await client.data.encrypt(original, {
    column: users.email,
    table: users,
  })
  expect(encrypted.failure).toBeUndefined()

  const decrypted = await client.data.decrypt(encrypted.data)
...
```typescript title="test/search.test.ts"
test("equality search finds encrypted record", async () => {
  // Encrypt and store a value
  const encrypted = await client.data.encrypt("alice@example.com", {
    column: users.email,
    table: users,
  })
  await pool.query(
  ...
  const query = await client.data.encryptQuery("alice@example.com", {
    column: users.email,
    table: users,
    queryType: "equality",
  })
...
If your CI uses Docker, ensure the native addon loads correctly by using a Linux-compatible lockfile. See [Bundling — Linux deployments](/docs/encryption/bundling#troubleshooting-linux-deployments) for details.
