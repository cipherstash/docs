...
Initialize the encryption client. Returns a `Result<EncryptionClient, EncryptionError>`.
...
if (client.failure) {
  console.error(client.failure.type, client.failure.message)
} else {
...
| `PROTECT_LOG_LEVEL` | — | Log verbosity: `debug`, `info`, `error` |
