...
CipherStash uses [keysets](/docs/kms/keysets) to scope encryption keys:
...
7. SDK packages ciphertext + index tokens into a [CipherCell](/docs/concepts/cipher-cell) (JSON payload).
...
4. PostgreSQL's [EQL extension](/docs/concepts/eql) compares encrypted tokens using the appropriate index — the database never sees plaintext.
...
For a deeper analysis of each index type, see [Searchable encryption](/docs/concepts/searchable-encryption) and [Supported queries](/docs/concepts/supported-queries).
...
CipherStash services are deployed across [multiple AWS regions](/docs/platform/regions). Key material does not leave the region your workspace is configured in.
...
- [What is CipherStash?](/docs/concepts/what-is-cipherstash) — platform overview and threat model
- [ZeroKMS](/docs/kms) — key management and keyset isolation
- [CipherCell format](/docs/concepts/cipher-cell) — encrypted payload structure
- [Disaster recovery](/docs/kms/disaster-recovery) — key recovery and outage behavior
