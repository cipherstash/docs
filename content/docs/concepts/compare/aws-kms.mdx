...
**Identity-aware encryption** — authenticate as the signed-in user and bind the data key to a claim from their JWT. AWS KMS has no built-in equivalent; encryption context is a manual workaround.
