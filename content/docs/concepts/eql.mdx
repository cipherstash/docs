...
**Encrypt Query Language (EQL)** is a set of PostgreSQL types, operators, and functions that enable queries on encrypted data without decryption. EQL works seamlessly with the [CipherCell](/docs/concepts/cipher-cell) format to provide searchable encryption capabilities directly in PostgreSQL.
...
The foundation of EQL is the `eql_v2_encrypted` data type, which stores [CipherCells](/docs/concepts/cipher-cell) containing encrypted data and searchable encrypted metadata.
...
[Learn more about exact indexes](/docs/concepts/supported-queries#exact-match)

### `ore` — Range queries

Enables range comparisons (`<`, `>`, `BETWEEN`) and ordering (`ORDER BY`) using Order Revealing Encryption.

[Learn more about range indexes](/docs/concepts/supported-queries#range--order)

### `match` — Pattern matching

Enables substring and full-text search (`LIKE`, `ILIKE`) using encrypted Bloom filters with trigrams.

[Learn more about match indexes](/docs/concepts/supported-queries#match-pattern)
...
EQL leverages PostgreSQL's native indexing capabilities to enable efficient queries on encrypted data. The searchable encrypted metadata within [CipherCells](/docs/concepts/cipher-cell) is indexed using standard PostgreSQL index types (B-tree for exact/range, GIN for pattern matching).
...
- [CipherCell format](/docs/concepts/cipher-cell) — The data structure used by EQL
- [Supported queries](/docs/concepts/supported-queries) — Available searchable encryption schemes
