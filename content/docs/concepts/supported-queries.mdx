...
Each index type stores its encrypted search terms in a specific key within the [CipherCell](/docs/concepts/cipher-cell) structure:
...
`Exact` indexes are stored in the `hm` key of a [CipherCell](/docs/concepts/cipher-cell).
...
`Match` indexes are stored in the `bf` key of a [CipherCell](/docs/concepts/cipher-cell).
...
`Order` / `Range` indexes are stored in the `ob` key of a [CipherCell](/docs/concepts/cipher-cell).
