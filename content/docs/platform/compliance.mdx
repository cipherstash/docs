...
| **GDPR** | Encryption of personal data, data residency controls via [regional deployment](/docs/platform/regions), crypto-shredding by deleting keysets |
...
Available regions are listed on the [Regions](/docs/platform/regions) page. When you create a workspace, you select a region and all ZeroKMS key material is stored and processed in that region.
...
| Identity context | The identity claim (if using [lock contexts](/docs/encryption/identity)) |
...
Because each [keyset](/docs/kms/keysets) provides cryptographic isolation, you can:
...
For rotation procedures, see [Keysets](/docs/kms/keysets).

## Security architecture

For a complete description of the cryptographic design, key hierarchy, and trust model, see [Security architecture](/docs/concepts/security-architecture).
