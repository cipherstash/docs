...
**Encrypt Query Language (EQL)** is a set of PostgreSQL types, operators, and functions that enable queries on encrypted data without decryption. EQL works seamlessly with the [CipherCell](/docs/platform/cipher-cell) format to provide searchable encryption capabilities directly in PostgreSQL.
...
When combined with the [Encryption SDK](/docs/encryption) or [CipherStash Proxy](/docs/proxy), EQL enables:
...
The foundation of EQL is the `eql_v2_encrypted` data type, which stores [CipherCells](/docs/platform/cipher-cell) containing encrypted data and searchable encrypted metadata.
...
[Learn more about exact indexes](/docs/platform/supported-queries#exact-match)

### `ore` — Range queries

Enables range comparisons (`<`, `>`, `BETWEEN`) and ordering (`ORDER BY`) using Order Revealing Encryption.

[Learn more about range indexes](/docs/platform/supported-queries#range--order)

### `match` — Pattern matching

Enables substring and full-text search (`LIKE`, `ILIKE`) using encrypted Bloom filters with trigrams.

[Learn more about match indexes](/docs/platform/supported-queries#match-pattern)
...
EQL leverages PostgreSQL's native indexing capabilities to enable efficient queries on encrypted data. The searchable encrypted metadata within [CipherCells](/docs/platform/cipher-cell) is indexed using standard PostgreSQL index types (B-tree for exact/range, GIN for pattern matching).
...
- [CipherCell format](/docs/platform/cipher-cell) — The data structure used by EQL
- [Supported queries](/docs/platform/supported-queries) — Available searchable encryption schemes
